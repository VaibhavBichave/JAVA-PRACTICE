	...
	public stock_entry() {
			...
			public void actionPerformed(ActionEvent arg0) {
				try {
					...
					PreparedStatement pst = con
							.prepareStatement("UPDATE equipment SET qty = ? WHERE Item_type=? and Item_name=?");
		...
		txt_total.addFocusListener(new FocusAdapter() {
			public void focusGained(FocusEvent arg0) {
				int total = Integer.parseInt(txt_qty.getText().toString())
		...
		item_name.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent arg0) {
				try {
					Connection con = DBConnect.getConnection();
					PreparedStatement pst = con
							.prepareStatement("select * from equipment where Item_type=? and Item_name=? ");
					pst.setString(1, comboBox.getSelectedItem().toString());
